...
const cancelEnrollment = async (req, res) => {
  try {
    ...
    console.log('Session businessId:', session.businessId);
    console.log('Current enrolled clients:', session.enrolledClients.map(e => e.clientId.toString()));

    // Determine the actual Client _id
    let actualClientId = clientId;
    
    // First, try to find enrollment with the provided clientId
    let enrollmentIndex = session.enrolledClients.findIndex(
      enrollment => enrollment.clientId.toString() === clientId.toString()
    );
    
    console.log('Initial enrollment index:', enrollmentIndex);
    
    // If not found and user is a client, try to find Client by userId
    if (enrollmentIndex === -1 && req.user.role === "client") {
      console.log('Enrollment not found with clientId, trying to find Client by userId...');
      ...
      if (!client) {
        console.log('Client not found with userId:', req.user._id, 'businessId:', session.businessId);
        return res.status(404).json({
          success: false,
          message: "Client record not found. Please ensure you are enrolled in this session."
        });
      }
      
      console.log('Found Client record:', client._id);
      actualClientId = client._id;
      
      // Try again with the actual Client _id
      enrollmentIndex = session.enrolledClients.findIndex(
        enrollment => enrollment.clientId.toString() === actualClientId.toString()
      );
      console.log('Enrollment index after Client lookup:', enrollmentIndex);
    }
    
    // For admin/instructor, verify business access
    if (req.user.role !== "client" && req.user.businessId.toString() !== session.businessId.toString()) {
      return res.status(403).json({
        success: false,
        message: "Access denied"
    ...
    // Get the clientId from the enrollment (should match actualClientId)
    const enrollmentClientId = session.enrolledClients[enrollmentIndex].clientId;
    ...
    // Get the client to check if this is their first cancellation
    const client = await Client.findById(enrollmentClientId);
    ...
    const isFirstCancellation = !client.hasCancelledBefore;
    
    // Increment cancellation counter and set hasCancelledBefore flag
    // If this is the first cancellation, set catch-up approval status to pending
    const updateData = {
      $inc: { cancellationCount: 1 },
      $set: { hasCancelledBefore: true }
    };
    
    // If this is their first cancellation, request catch-up approval
    if (isFirstCancellation) {
      updateData.$set.catchUpApprovalStatus = 'pending';
    }
    
    await Client.findByIdAndUpdate(enrollmentClientId, updateData);
    
    // Create notification for business dashboard
    ...
    // If this is first cancellation, create notification for catch-up approval request
    if (isFirstCancellation) {
      await Notification.create({
        businessId: session.businessId,
        type: "catch-up-approval-request",
        message: `${client.firstName} ${client.lastName} has cancelled a lesson and is requesting catch-up access`,
        clientId: enrollmentClientId,
        sessionId: session._id
      });
    }
    
    // Create cancellation notification with pending catch-up approval status
    await Notification.create({
      businessId: session.businessId,
      type: "cancellation",
      message: `${client.firstName} ${client.lastName} cancelled ${classInfo.title}`,
      clientId: enrollmentClientId,
      sessionId: session._id,
      catchUpApprovalStatus: 'pending' // Each cancellation gets its own approval status
    });
...
const getCatchUpLessons = async (req, res) => {
  try {
    ...
    // Check if client has cancelled before
    const client = await Client.findOne({ 
      userId: userId, 
    ...
    if (!client.hasCancelledBefore) {
      return res.json({
        ...
        count: 0,
        data: [],
        message: "Catch-up lessons are only available to clients who have previously cancelled a session"
      });
    }

    // Check if client has approved catch-up access
    if (client.catchUpApprovalStatus !== 'approved') {
      const statusMessage = client.catchUpApprovalStatus === 'pending' 
        ? "Your catch-up access is pending approval by the business administrator"
        : client.catchUpApprovalStatus === 'rejected'
        ? "Your catch-up access has been rejected. Please contact the business administrator"
        : "Your catch-up access requires approval by the business administrator";
